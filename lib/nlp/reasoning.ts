...
const COMBINATION_RULES: CombinationRule[] = [
  ...
  },
  {
    ...
    conditions: [["panas", "uv_tinggi"], ["panas", "uv_sangat_tinggi"]],
    severity: 0.3,
  ...
  // ── Visibilitas + Kondisi ────────────────────────────────────────────────
  {
    ...
    conditions: [["jarak_pandang_buruk", "kondisi_fog"]],
    severity: 0.35,
...
function escalateSentiment(
  ...
  if (maxSeverity >= 0.35 && (original === "baik" || original === "cukup")) return "waspada"
  return original
}
 ...
 */
export function reasonWeather(concepts: StemmedConcept[]): ReasoningResult {
  ...
  const adjustedWeights = concepts.map((c) => {
    ...
    return Math.min(c.weight + boost, 1.0)
  })

  const avgWeight = adjustedWeights.length > 0
    ? adjustedWeights.reduce((s, w) => s + w, 0) / adjustedWeights.length
    : 0
  const adjustedScore = Math.round(avgWeight * 100)

  const adjustedSentiment = escalateSentiment(
    concepts.some((c) => c.sentiment === "berbahaya") ? "berbahaya" :
    concepts.filter((c) => c.sentiment === "waspada").length / concepts.length >= 0.5 ? "waspada" :
    adjustedScore >= 75 ? "baik" : "cukup",
    maxSeverity
  )
