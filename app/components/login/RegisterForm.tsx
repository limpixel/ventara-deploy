...
export default function RegisterForm({ onRegister, onSwitch }: Props) {
  ...
  return (
    <div className="relative w-full h-full overflow-hidden bg-[linear-gradient(135deg,#163832_0%,#1f4d45_28%,#2d6a5f_58%,#3f8f7f_100%)] flex justify-center items-center p-10 px-14">
      ...
      {/* FORM */}
      <form
        onSubmit={(e) => { e.preventDefault(); onRegister(username, email, password); }}
        className="relative z-10 w-full max-w-105"
      >

        {/* TITLE */}
        <h1 className="text-[32px] font-bold text-white mb-2">Create Account</h1>
        <p className="text-sm text-white/65 mb-8">Join Ventara and explore future insights.</p>
        ...
        <p className="mt-6 text-center text-sm text-white/65">
          ...
          <button type="button" onClick={onSwitch} className="text-white font-bold hover:underline">
            Log in
          </button>
        </p>
      </form>
